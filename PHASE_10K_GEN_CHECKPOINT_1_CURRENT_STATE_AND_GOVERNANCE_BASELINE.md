...
| `TD-CATALOG-LEGACY-RESOLVER-STATUS-CONFLATION-001` | `CatalogStatus` alone answered two separate questions (artifact validity vs. implicit bare-key legacy-resolver default eligibility) until `FREQ.6D.4C.5` added a narrow, additive `WorkoutDefinition.EligibleForLegacyDefaultResolution` field scoped only to `CatalogSourceSnapshot.FindWorkout(string, IRetirementLedger?)`. Narrowly closed for `AEROBIC_STRENGTH_CONTROLLED_INTRO v3`/`THRESHOLD_TEMPO v5`/`FARTLEK v5`/`GOAL_PACE_TEN_K v3` (all explicitly `false`); the legacy resolver itself remains in place for historical-replay combinations `v1`-`v3` and any future artifact facing the same narrow-addition-vs-default tension would need the same explicit opt-out | `FREQ.6D.4C.4 §15`, closed for these 4 versions by `FREQ.6D.4C.5` | Narrowly closed for these 4 versions; general conflation mechanism remains available/open for future use |
