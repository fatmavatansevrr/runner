...
**2D axis binding blocker — escalated** (as of `GEN.13`, `DOMAIN_DECISION_REQUIRED`): `GEN.12`'s own disclosed `ProgressionStageAllocator` lane/week-eligibility gap was classified, not resolved — confirmed no existing precedent anywhere in this codebase handles a lane with less than 100% weekly structural coverage (every existing dual-KEY lane, in every existing frequency, is present in every phase week), and confirmed the choice genuinely changes what an athlete is prescribed (how many times, and in which weeks, a quality-session stage is scheduled) — a training-methodology decision, not an implementation detail. Three options were presented, none implemented, none recommended over another: (A) reinterpret exposure capacity against the Pattern-A-only week subset (likely breaks the existing 100%-coverage-calibrated minimums for short phases); (B) author new 2D-specific exposure content calibrated for a Pattern-A-only cadence (a real new coaching decision); (C) weaken the existing "minimum exposures always honored" safety guarantee to best-effort for 2D (a real prescription-quality regression risk). **Phase F (completing 2D's remaining scope — binding, volume/long-run planning, Adaptation, Runway, LongHorizon) does not proceed until a human decision is made.**

**2D axis Core implementation status** (as of `GEN.12`, `DONE (PARTIAL)`): closed `GEN.11`'s first-disclosed architecture gap with an additive, zero-delta repeating-pattern mechanism (`PlanCatalogCandidateSummary.WeeklyPatternRoles`/`PatternPeriodWeeks`, both `null` for every pre-GEN.12 candidate), authored `RUN_LAYOUT_2D` and both new `TEN_K__2D__BEGINNER`/`TEN_K__2D__INTERMEDIATE` combinations (single-KEY, zero new prescription content, reusing the existing single-lane `TEN_K_MASTER v6` catalog lineage rather than the dual-KEY v9/v10 one), implemented `VolumeSafetyPolicy.Beginner2D`/`Intermediate2D` and the frequency-owned missing/zero-readiness exception per `GEN.11`'s exact frozen values. Found and fixed 3 real defects, all "assumed every week has ≥1 KEY_SESSION" hardcodes exposed for the first time by 2D's zero-KEY_SESSION Pattern-B week (a combinatorics `k==0` edge case and two sibling role-count validators) — dark-verified via 11 new tests proving real structural skeleton generation and calendar day-assignment for both levels, plus zero-delta for existing frequencies. **Explicitly not yet implemented**: workout-content binding (`ProgressionStageAllocator` allocates progression-stage exposure across every literal calendar week with no concept of a week lacking a KEY_SESSION slot for a given lane — a genuine architecture question requiring its own dedicated design pass, correctly disclosed rather than patched around), and everything downstream of it (full volume/long-run planning, Adaptation, Preparation Runway, LongHorizon). Public gate untouched throughout. Next: the `ProgressionStageAllocator` lane/week-eligibility design phase, then full Core dark verification, then Runway, then LongHorizon — not yet scheduled as a Phase ID.
