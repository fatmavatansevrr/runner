...
| **Advanced** | `FULL_IMPLEMENTATION_AUTHORITY_COMPLETE` (GEN.7/GEN.8 — not yet implemented) | `FULL_IMPLEMENTATION_AUTHORITY_COMPLETE` (GEN.7/GEN.8 — not yet implemented) | `FULL_IMPLEMENTATION_AUTHORITY_COMPLETE` (GEN.7/GEN.8 — dual-KEY content frozen, not yet implemented) | `FULL_IMPLEMENTATION_AUTHORITY_COMPLETE` (GEN.7/GEN.8 — reuses 5D's dual-KEY content, not yet implemented) | `PRODUCT_NON_SUPPORT` (GEN.7 — inherited frequency-global calendar gap) |
...
**Advanced axis authority status** (as of `GEN.8`, superseding `GEN.7`'s two disclosed gaps): unlike Beginner, Advanced's two-KEY decision was approved — Appsel's own real, once-`VALIDATED` historical `PEAK_VOLUME_BANDS_V1.v1/v2` Advanced×5D row (`[42,58]`) already modeled a dual-KEY product, and no injury-risk/coaching-convention concern applies to the advanced-tier population. **Both of `GEN.7`'s disclosed gaps are now closed**: (1) missing-readiness → `PRODUCT_INELIGIBLE`, a direct extension of the already-approved zero-readiness rule, not an invented default; (2) Advanced×5D/6D dual-KEY prescription content — 8 new profile documents frozen (`ADVANCED_FOUNDATION_PRIMARY` through `ADVANCED_TAPER_SECONDARY_CONTROLLED`), reusing Intermediate's exact structural dose verbatim since intensity differentiation is already automatic via each runner's own resolved pace, requiring zero new WorkoutDefinition content and no architecture change. **Advanced×3D/4D/5D/6D are all `FULL_IMPLEMENTATION_AUTHORITY_COMPLETE`.** Advanced×7D remains `PRODUCT_NON_SUPPORT` (inherited frequency-global calendar gap). Recommended next step: one combined Advanced 3D-6D implementation/dark-verification wave, then one combined public-activation phase — not yet scheduled as a Phase ID.
...
- Complete Advanced Level across proven frequencies. **AUTHORITY COMPLETE** (`GEN.7`/`GEN.8`) — 3D/4D/5D/6D all `FULL_IMPLEMENTATION_AUTHORITY_COMPLETE`; 7D `PRODUCT_NON_SUPPORT`. Implementation not yet started.
