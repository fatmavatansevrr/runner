...
1. **2D LongHorizon** (Preparation Runway itself now dark-implemented and dark-verified as of `GEN.29`; architecture/precedent-mapping now performed by `GEN.30`, user-authored "PHASE PROMPT 06c," Step 1): `GEN.30` mapped every LongHorizon mechanism 2D needs against the real, ~12-phase `FREQ.6D.11`-`22` Intermediate×5D precedent. **Ten of eleven mechanisms resolve favorably as mechanical reuse/extension** — most notably, the dual-KEY session-identity/lineage architecture (`FREQ.6D.11`'s single hardest, most expensive design item) is **structurally inapplicable** to 2D (Model B never produces two same-role sessions in a week), and the Runway→Core numeric-continuity clamp (`FREQ.6D.16`/`.17`, the hardest numeric defect in the 5D arc) is already live, generic, and frequency-neutral in production, free for 2D to inherit. **One mechanism is genuinely different and is escalated as `DOMAIN_DECISION_REQUIRED`**: the LongHorizon GE segment's own weekly session-role structure — every prior GE precedent (4D, 5D) had a non-alternating Core/Runway, so "GE mirrors the literal 4D/5D GE shape" and "GE mirrors this frequency's own Runway/Core shape" were always the same statement; 2D's Model B Runway/Core alternates (Pattern A/B), so these become two different, mutually exclusive options for the first time — Option A (GE inherits the alternating Model B structure unmodified, direct symmetric extension of `GEN.26 §2`) vs. Option B (GE uses a fixed constant-KEY-every-week shape, the literal 4D/5D precedent, which would mean 2D's quality-session frequency *decreases* at the GE→Runway boundary, a discontinuity direction never previously evaluated in this engagement). `GEN.30` records a disclosed, non-binding recommendation for Option A but does not freeze it. A future decision phase resolving this, followed by a combined implementation+dark-verification phase (analogous in shape to `FREQ.6D.13`/`.14`) building the ten already-resolved mechanisms plus the now-decided GE structure, remain the two next steps — neither scheduled here. Preparation Runway's own **public** HTTP/PostgreSQL activation for 2D also remains a separate, not-yet-scheduled follow-on.
