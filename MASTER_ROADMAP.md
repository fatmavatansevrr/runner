...
|               | 2D | 3D | 4D | 5D | 6D | 7D |
|---|---|---|---|---|---|---|
| **Beginner** | `TWO_D_BEGINNER_INTERMEDIATE_CORE_PUBLICLY_ACTIVE_10_14_WEEKS` (GEN.20) — Core `PUBLICLY_ACTIVE`, real HTTP/PostgreSQL, exactly 10-14wk; 8-9wk formally, permanently non-supported (`TWO_D_CORE_EIGHT_AND_NINE_WEEK_NON_SUPPORT_FORMALIZED_FINAL`, GEN.18, now real-HTTP-verified fail-closed with a GEN.18-citing typed rejection); Preparation Runway (15-20wk)/LongHorizon (21-52wk) remain **not implemented** — GEN.19 confirmed a dedicated Runway repeating-pattern architecture-design phase is required before either can proceed; separately-budgeted future work comparable in scope to the `FREQ.6D.11`-`FREQ.6D.22` arc (~12 phases), not a small follow-on | `BEGINNER_3D_SUPPORTED_EXPLICIT_ZERO_PRODUCT_INELIGIBLE` (Core only; GEN.23 implemented the user's frozen Option-1 taper-minimum decision on GEN.21's escalation, GEN.24 then resolved GEN.23's own disclosed explicit-zero gap by direct user decision) — Beginner×3D is `SUPPORTED`, not a frequency-level non-support classification. Real, dark, full-pipeline-verified: missing-readiness and positive-observed readiness are `ELIGIBLE`/representable at every governed Core horizon (8-14wk, re-verified zero-delta by GEN.24); explicit-zero readiness is `PRODUCT_INELIGIBLE` — a formal, typed, request-level rejection (`BeginnerThreeDayExplicitZeroReadinessProductIneligibleException`, mirroring GEN.9's Advanced missing/zero mechanism class), not a silent fallback or a frequency-level non-support reclassification; no starting-volume default was raised or invented. **Internally gated only — `V1CatalogPilotIdentityPolicy`'s public allow-list was deliberately NOT widened**; public HTTP activation is a separate, not-yet-scheduled future phase, mirroring `GEN.4D`→`GEN.4E`. `BEGINNER_3D_CORE_NON_SUPPORT_FORMALIZED_FINAL`'s (`GEN.5C`) text is unmodified; its characterization is superseded (true of the old undifferentiated-floor policy, no longer blocking missing/positive-observed readiness under the new one; explicit-zero is now a request-level readiness ineligibility, not a representability blocker) | `PUBLICLY_ACTIVE` (Core; GEN.4E) | `PRODUCT_NON_SUPPORT` (GEN.6 — two-KEY Core eligibility rejected; reaffirmed under the SECONDARY_CONTROLLED framing by GEN.22 — evidence base too thin to specify a concrete non-hard second lane) | `PRODUCT_NON_SUPPORT` (GEN.6 — same as 5D) | `PRODUCT_NON_SUPPORT` (GEN.6 — inherited frequency-global calendar gap) |
