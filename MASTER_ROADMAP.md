...
- Complete Beginner remaining frequencies. **DONE** (`GEN.6`) — 5D/6D/7D all `PRODUCT_NON_SUPPORT`; Beginner remains 4D-only. `BEGINNER_FREQUENCY_AUTHORITY_COMPLETE`. Realigned under a later product requirement (`GEN.21`): Beginner×3D `DOMAIN_DECISION_REQUIRED` (real mutable lever confirmed, exact resolution undecided). Beginner still remains 4D-only pending that decision.
