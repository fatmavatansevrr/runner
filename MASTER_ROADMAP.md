...
**Latest verified completed phase**: `FREQ.6D.4D.2` (Split B) — Execution Status `DONE`, Final Classification `FREQ6D4D_SPLIT_B_IMPLEMENTED_ADAPTATION_ENGINEERING_GAP_REMAINS`. Implemented exact prescription-profile resolution + projection-dependency materialization + bundle wiring: additive `PrescriptionProfileCandidates`/`PrescriptionProfileCandidateKeys` on the progression-stage model (PlanCatalog.Core + RunningApp mirror); cardinality-only exact resolution in `CatalogWorkoutBinder` (0→Legacy, 1→ProfileBacked, >1→fail-closed typed exception); `PrescriptionProfileLaneDoseValidator` reused verbatim for the LaneOrdinal↔DoseCategory invariant; new `PrescriptionProfileClosureResolver`/`PrescriptionProjectionDependencyResolver` glue feeding the unmodified `FREQ.6D.3C` `CatalogBundleAssembler` exact-dependency overload, proven to produce non-null, deterministic, deduplicated `ExecutionPrescriptions` for a dual-lane profile-backed combination. Caught and fixed two real, disclosed gaps while wiring: a `[JsonIgnore]` hash-stability fix (a new computed `EffectiveLanes` property was silently perturbing every published document's content hash) and a lane-awareness fix to `WorkoutClosureResolver`/`CatalogBundleAssembler` (both read `.Stages` directly, ignoring lane-authored content). 21 new tests; PlanCatalog full regression 1,501/1,501; RunningApp's DB-backed integration subset could not execute in this environment (Docker/Postgres unavailable, confirmed via `docker ps`; all 197 non-executing failures independently verified as the identical Npgsql-connection signature, zero logic/assertion failures among them) — disclosed as an environment limitation, not a code regression. Real public 5D catalog content (an actual `RUN_LAYOUT_5D`/combination) remains Split E's job per the architecture's own decomposition.

**Next phase per repository-backed sequencing**: Split C — RunningApp session lineage (`CatalogSessionPrescriptionPlanner` reading `BoundCatalogSession.PrescriptionProfileKey`/`Version`; `CatalogSessionPrescriptionSource` actually wired into the live session-building branch), consuming the exact profile lineage Split B now materializes on every `ProfileBacked BoundCatalogSession`, per `FREQ.6D.4D §41`'s dependency-ordered decomposition. Split D (persistence/Adaptation integration — may run in parallel with C) and Split E (integrated closure, including the still-absent `RUN_LAYOUT_5D` catalog authoring) follow. Dual-KEY production integration itself remains **not implemented**.
...
[Splits C-E, not yet scheduled] → RunningApp session lineage; persistence/Adaptation integration
                                    (incl. 5D severity-table fix, 2 new TrainingDay columns);
                                    integrated closure + RUN_LAYOUT_5D authoring
