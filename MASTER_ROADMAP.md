...
**Latest verified completed phase**: `FREQ.6D.4D.5D` — Execution Status `DONE (PARTIAL)`, Final Classification `FREQ6D4D5D_TAPER_FIXED_PUBLIC_ACTIVATION_BLOCKED_ELSEWHERE`. Implemented the approved `FREQ.6D.4D.5C` Legacy/ProfileBacked Taper-completeness partition in `CatalogPrescriptionContextValidator` (additive `PrescriptionProfileKey`/`Version` threading), and found + fixed a second, previously-undisclosed occurrence of the identical root cause in `CatalogFinalPrescribedPlanValidator` (using the already-existing `CatalogPrescribedSession.PrescriptionSource` classification). Both real 5D Taper lanes (`TAPER_PRIMARY_STAGE`/`TAPER_SECONDARY_STAGE`) proven valid through both validators without stage-name special-casing; the `FREQ.6D.4D.5C` malformed-Legacy counterexample proven still rejected; zero legacy delta (3,649/3,650 full regression, 1 pre-existing unrelated failure; 1,510/1,510 PlanCatalog). Retried public activation a third time: real E2E testing confirmed both Taper fixes work — the request progressed cleanly past calendar and both Taper checks for the first time — then found a third, genuinely independent, out-of-scope blocker: `V1CatalogPublicWorkoutTypeMappingPolicy` has no public workout-type mapping for the real 5D `AEROBIC_STRENGTH_CONTROLLED_INTRO` workout. Reverted the widening a third time rather than guess at the mapping. `TEN_K__5D__INTERMEDIATE` remains fully dark to public traffic.
...
[Next, not yet scheduled]       → resolve V1CatalogPublicWorkoutTypeMappingPolicy's scope/provenance
                                    for AEROBIC_STRENGTH_CONTROLLED_INTRO (audit for any other real
                                    5D workout key with the same gap) -- a real product/evidence
                                    question -- then re-attempt 8-14w public activation a fourth
                                    time; Preparation Runway (15-20w) / Long-Horizon (21-52w) 5D
