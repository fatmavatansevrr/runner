...
**Next phase per repository-backed sequencing**: thread `IPublishedTemplateBundleLoader`/`ExecutionPrescriptionIndex` into `CatalogPreviewGenerator`'s `CompressedCore`/`ExtendedCore` dynamic-orchestration branch, mirroring how the exact-12-week "preferred" pipeline already does it — a narrow, unambiguous engineering fix (not a product/evidence decision). Once fixed and regression-proven across 8/10/12/14 weeks, retry public activation of `TEN_K__5D__INTERMEDIATE` a fifth time (same routing widening as Split E/`5B`/`5D`/`5F`, no redesign). Dual-KEY production integration itself remains **not implemented** until that fifth attempt succeeds.
