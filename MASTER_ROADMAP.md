...
| **Advanced** | `AUTHORITY_APPROVED_IMPLEMENTATION_READY` (GEN.7 — not yet implemented) | `AUTHORITY_APPROVED_IMPLEMENTATION_READY` (GEN.7 — not yet implemented) | `AUTHORITY_APPROVED_WITH_PRESCRIPTION_CONTENT_BLOCKER` (GEN.7 — dual-KEY dose content required before implementation) | `AUTHORITY_APPROVED_WITH_PRESCRIPTION_CONTENT_BLOCKER` (GEN.7 — same as 5D) | `PRODUCT_NON_SUPPORT` (GEN.7 — inherited frequency-global calendar gap) |
...
**Advanced axis authority status** (as of `GEN.7`): unlike Beginner, Advanced's two-KEY decision was approved — Appsel's own real, once-`VALIDATED` historical `PEAK_VOLUME_BANDS_V1.v1/v2` Advanced×5D row (`[42,58]`) already modeled a dual-KEY product, and no injury-risk/coaching-convention concern applies to the advanced-tier population. Advanced×3D/4D are fully authority-closed and implementation-ready (reusing Level-agnostic workout content, zero new catalog needed). Advanced×5D/6D are structurally/eligibility-approved but blocked on one narrow item: the existing dual-KEY prescription-profile documents are genuinely Level-specific (`INTERMEDIATE_5D_*`), so Advanced needs its own dose-content authored before implementation — `CATALOG_CONTENT_AUTHORITY_REQUIRED`. Advanced×7D inherits the frequency-global calendar gap, `PRODUCT_NON_SUPPORT`. One item (Advanced missing-readiness starting-volume default) is honestly left `DECISION_REQUIRED` rather than invented. Recommended next step: a narrow Advanced 5D/6D prescription-content-authority closure, then one combined 3D-6D implementation wave — not yet scheduled as a Phase ID.
...
- Complete Advanced Level across proven frequencies. **AUTHORITY CLOSED** (`GEN.7`) — 3D/4D implementation-ready; 5D/6D authority-approved but blocked on prescription-content authoring; 7D `PRODUCT_NON_SUPPORT`. Implementation not yet started.
