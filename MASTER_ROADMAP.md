...
1. **2D LongHorizon** (Preparation Runway itself now dark-implemented and dark-verified as of `GEN.29`; architecture/precedent-mapping performed by `GEN.30`; GE-segment structure decision now resolved by `GEN.31`): `GEN.30` mapped every LongHorizon mechanism 2D needs against the real, ~12-phase `FREQ.6D.11`-`22` Intermediate×5D precedent — ten of eleven mechanisms resolve favorably as mechanical reuse/extension (the dual-KEY session-identity/lineage architecture, `FREQ.6D.11`'s single hardest design item, is **structurally inapplicable** to 2D; the Runway→Core numeric-continuity clamp is already live, generic, and free for 2D to inherit), leaving exactly one mechanism — the GE segment's own weekly session-role structure — genuinely undecided between Option A (GE inherits the alternating Model B structure unmodified, continuing the same `GlobalWeekNumber` from GE Week 1 through Runway and Core) and Option B (a fixed constant-KEY-every-week shape copying 4D/5D's literal GE precedent). **`GEN.31` closes this decision by direct user authority: Option A is approved; Option B is formally rejected, not implemented, not partially borrowed from.** `GEN.31` also resolved the governing prompt's required GE-content-needs question, finding `GE_CONTENT_REUSE_CONFIRMED_NO_FRESH_AUTHORING_REQUIRED` — GE's own stage-family catalog table already carries independently-authored `KEY_SESSION` and `EASY_SUPPORT` role assignments for every stage family (unlike Runway's pre-`GEN.28`/`.29` block table, which needed a new `EASY_SUPPORT` alternate added), so Option A's alternation needs zero new `WorkoutDefinition` and zero new content mechanism — only a selection between two already-existing references by week pattern. `GEN.31` did **not** attempt Step 2 implementation (structural-selector generalization, ~20-site admission-gate/JIT/persistence widening, GE→Runway handoff proof, full dark verification) — disclosed explicitly as genuinely multi-phase scope, re-confirmed (not merely re-cited) by a direct re-read of the current `RollingActivation/` surface, and left as a concrete six-item implementation contract for the next, not-yet-scheduled phase. Preparation Runway's own **public** HTTP/PostgreSQL activation for 2D also remains a separate, not-yet-scheduled follow-on.
