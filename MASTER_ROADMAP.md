...
| **Intermediate** | `PUBLICLY_ACTIVE` (Core; GEN.3B) | `PUBLICLY_ACTIVE` (pre-existing/Adaptation V1 baseline) | `PUBLICLY_ACTIVE` (Core, 8-14 weeks; FREQ.6D.4D.5G). **Preparation Runway (15-20w) now `PUBLICLY_ACTIVE` (FREQ.6D.8)** — real HTTP + real PostgreSQL verified: all six 15-20 week horizons return 200 with exact `TEN_K__5D__INTERMEDIATE` identity, exact 1 KEY + 3 EASY + 1 LONG Runway shape, exact 2 KEY + 2 EASY + 1 LONG Core Week 1, second KEY only there; persisted structure/boundary/profile-lineage confirmed; zero delta to Intermediate×4D Runway or Intermediate×5D Core-only. A separate, disclosed, pre-existing Core-side defect (missing/explicit-zero starting-volume evidence fails real Core generation at the 5D 2-KEY minimum — also present in the already-active Core-only route, not introduced by Runway) remains open for a future narrow phase; it does not block this capability. **Long-Horizon (21-52w) 5D remains `BLOCKED` on a separate, deeper architecture/schema gap** (`LongHorizonRollingSessionState` has no lane/stage/profile lineage columns; real JIT Core-week composition discards dual-KEY lineage) — a distinct, later, unscheduled wave per `FREQ.6D.5` | not yet opened | not yet opened |
...
**Latest verified completed phase**: `FREQ.6D.8` — Execution Status `DONE`, Final Classification `INTERMEDIATE_5D_PREPARATION_RUNWAY_IMPLEMENTED_AND_PUBLICLY_ACTIVATED`. Closed `FREQ.6D.7`'s own disclosed gap: real HTTP E2E (real Api host, real Postgres, no mocks) for all six 15-20 week Intermediate×5D Preparation Runway public previews, all 200 with exact identity/structure; real confirmation and persistence verified for 15/17/20-week horizons including a fresh-DB-reload permanent Runway→Core boundary regression and real profile lineage; unsupported neighbors and 21+/24-week LongHorizon remain closed; Intermediate×4D Runway and Intermediate×5D Core-only remain byte-for-byte unchanged. Found and disclosed (not fixed, per explicit scope) a real, pre-existing Core-side defect: missing/explicit-zero starting-volume evidence fails real Core generation for Intermediate×5D at the real 2-KEY minimum — independently proven pre-existing by reproducing identically against the already-`PUBLICLY_ACTIVE` Core-only route and against the pre-`FREQ.6D.7` durable baseline commit. No routing change was needed (already wired); no production code touched, only two new real-environment test files. Full regression 3744/3746 (2 pre-existing failures, both re-verified against the durable baseline), 1510/1510 PlanCatalog. **Intermediate×5D Preparation Runway (15-20 weeks) is now genuinely publicly active.** Next: LongHorizon architecture-design, or a future narrow phase for the disclosed Core starting-volume gap.

Prior phase: `FREQ.6D.7` — Execution Status `DONE`, Final Classification `INTERMEDIATE_5D_PREPARATION_RUNWAY_IMPLEMENTED`. Generalized the Intermediate×4D Preparation Runway implementation to also support Intermediate×5D, reusing the existing Runway engine and existing generic Core pipeline (no second implementation, `RUN_LAYOUT_5D` untouched). Fixed every `FREQ.6D.5`-identified Runway hardcoding plus several more discovered only during implementation. Public routing was code-wired but real HTTP E2E and real PostgreSQL confirmation were not performed — the gap `FREQ.6D.8` closed.
...
**Next phase per repository-backed sequencing**: either (a) a narrow future phase to resolve the `FREQ.6D.8`-disclosed pre-existing Core defect (missing/explicit-zero starting-volume evidence infeasible at the real Intermediate×5D 2-KEY minimum — affects both the Core-only and Runway+Core public routes, requires new numeric authority) — or (b) proceed directly to LongHorizon's own architecture-design phase (persisted-schema lineage gap: no `LaneOrdinal`/`ProgressionStageKey`/`PrescriptionProfileKey` columns on `LongHorizonRollingSessionState`; JIT dual-KEY lane-lineage loss in `BuildBoundedCoreSelection`; `ExecutionPrescriptionIndex` propagation into the rolling-activation paths; the ~10 `DaysPerWeek==4` gates `FREQ.6D.5` §12 identified), since both share the same underlying `TenKPreparationRunwayDarkOrchestrator` Core-entry and Preparation Runway is now fully publicly active.
...
**WAVE A — 10K completion.** Intermediate×5D Core (8-14 weeks) is `PUBLICLY_ACTIVE` (FREQ.6D.4D.5G, §2). Preparation Runway (15-20w) is now `PUBLICLY_ACTIVE`, real HTTP/DB verified (FREQ.6D.8, §2). Long-Horizon (21-52w) 5D activation remains an open, not-yet-scheduled gap, and a narrow disclosed Core starting-volume defect (FREQ.6D.8 §16-17) remains open. No Half Marathon or Marathon work may begin under this roadmap's own rule until 10K's full architectural closure (§25/Wave A milestones, including the Long-Horizon 5D gap) is reached.
...
[Next, not yet scheduled]       → A narrow future phase for the FREQ.6D.8-disclosed pre-existing Core
                                    starting-volume defect, or LongHorizon architecture-design
