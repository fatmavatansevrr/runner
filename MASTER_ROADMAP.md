...
| **Intermediate** | `PUBLICLY_ACTIVE` (Core; GEN.3B) | `PUBLICLY_ACTIVE` (pre-existing/Adaptation V1 baseline) | `BLOCKED` — product policy approved (FREQ.6), numeric authority approved (FREQ.6C), catalog architecture design verified (FREQ.6D.1/1A/1B), engineering machinery (schema/projector/RunningApp consumer) implemented through FREQ.6D.3D; recovery-ownership architecture resolved (FREQ.6D.4B.2/4B.4) and all 8 real production WorkoutPrescriptionProfile documents now authored (FREQ.6D.4C.3); catalog-lifecycle/exact-version-activation architecture approved and implemented (FREQ.6D.4C.4/4C.5) — all four required WorkoutDefinition versions are VALIDATED and exact-reference-usable; **dual-KEY stage→profile production-integration architecture approved (FREQ.6D.4D)** — Week×LaneOrdinal→exact stage→exact profile chain fully designed; slot/lane identity + per-lane stage binding (FREQ.6D.4D.1), exact profile resolution + bundle wiring (FREQ.6D.4D.2), RunningApp session-lineage consumption (FREQ.6D.4D.3), and durable profile-lineage persistence + the complete 5-session Adaptation severity table (FREQ.6D.4D.4) now implemented and real-DB-verified, but real public 5D catalog content and activation (Split E) remain **not yet implemented**; **not publicly active** | not yet opened | not yet opened |
...
**Latest verified completed phase**: `FREQ.6D.4D.4` (Split D) — Execution Status `DONE`, Final Classification `FREQ6D4D_SPLIT_D_IMPLEMENTED_SPLIT_E_READY`. Restored the DB verification environment (Docker Desktop + the repository's own `docker-compose` dev Postgres) and ran the entire split against real PostgreSQL. Added two new nullable `TrainingDay` columns (`CatalogPrescriptionProfileKey`/`Version`) via a real, applied EF migration; wired both live confirmation mappers (`CatalogPublicPreviewMaterializer`, `PreparationRunwayPersistablePlanMapper`) to thread `CatalogPrescribedSession.PrescriptionSource`'s exact profile lineage into `TrainingDay` — proven via 5 real-Postgres round-trip tests (legacy null, dual-lane same-stage-different-profile, version stability, completion preservation, historical-row readability). `LaneOrdinal`/execution-content persistence deliberately not added, per the real architecture's own already-resolved authority (derivable / bundle-only). Audited the schedule-repair/substitution subsystem and found it already correct by construction, per `FREQ.6D.1B`'s own resolved recommendation — no change required. Implemented the complete, real, frozen `FREQ.6 §6` 24-row five-session Adaptation severity table in `NextWindowLoadDecisionPolicy`, dispatched only for a genuine 5-session week, with legacy 4-session behavior byte-for-byte preserved; 44 new tests, 24/24 rows verified, one genuine pre-existing test-data bug found and fixed along the way. Full regression: 2,967/2,969 RuntimeCatalog (2 pre-existing unrelated inventory-count failures), 1,501/1,501 PlanCatalog, 192/192 LongHorizon.Adaptation.
...
[Split E, not yet scheduled]    → integrated closure: real RUN_LAYOUT_5D/combination authoring,
                                    public 5D activation, published-bundle file-discovery wiring
                                    into PlanCatalogBundleLoader
