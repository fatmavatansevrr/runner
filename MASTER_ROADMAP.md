...
1. **2D LongHorizon** (Preparation Runway itself now dark-implemented and dark-verified as of `GEN.29` — see the 2D axis entry below): GEN.28 §13's five-item LongHorizon start-gate is now met (Runway materializes all four blocks correctly for both A/B patterns and both levels; the `NotSupportedException` guard is gone; Beginner/Intermediate admission both work; numeric dispatch and calendar/long-run-clamp wiring are both real-verified), so a dedicated LongHorizon 2D phase may now begin — still budgeted comparable in scope to the `FREQ.6D.11`-`FREQ.6D.22` arc (~12 phases) for the real rolling-activation/persistence/adaptation/repair effort, not a small follow-on. Preparation Runway's own **public** HTTP/PostgreSQL activation for 2D also remains a separate, not-yet-scheduled follow-on (this phase stayed dark-only per its own scope).
