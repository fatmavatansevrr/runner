...
### Revizyon 4 — Maintain/Reduce Numeric Anchor Semantics (ReduceBand çözüldü)
...
**Severity-ordering invariant (DECIDED, test edilecek):**
```
...
zaman Maintain'i sayıca geçemez. `NextWindowTargetAnchor(Maintain) ≤
NextWindowTargetAnchor(ProgressAsPlanned)` tarafı ise
`CatalogProgressionStep`'in monoton olmayan (plato/regresyon) davranışı
olabileceğinden **varsayılmaz, testle doğrulanır** (BACKLOG değil,
4M.4B.2'nin test kapsamı).
