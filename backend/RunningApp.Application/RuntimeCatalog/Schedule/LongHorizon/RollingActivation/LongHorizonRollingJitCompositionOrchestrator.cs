        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (needsCoreGeneration)
            {
                ...
                // Phase 10K-GEN.37 -- "DECISION ON GEN.36" resolved the pace-anchor conflict
                // GEN.36 disclosed (PreparationRunwayCoreWeekOnePaceAdapter now anchors to the
                // first Core week within local weeks 1-2 that carries a KEY_SESSION, instead of
                // unconditionally assuming local week 1 does). CoreStartGlobalWeek can now be
                // safely enabled here, mirroring runwayStartGlobalWeek's own convention exactly
                // -- both consumers of "where is Core's week 1 in the global sequence" agree by
                // construction (BuildBoundedCoreSelection already used coreSegment.StartGlobalWeek
                // to stamp GlobalWeekNumber; the real content generator now continues the same
                // global parity instead of always restarting local week 1 at Pattern A).
                var compositionRequest = new TenKPreparationRunwayDarkOrchestrationRequest(
                    ...
                    CoreStartGlobalWeek: coreSegment.StartGlobalWeek);
