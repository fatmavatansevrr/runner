...
/// <summary>
/// PHASE DIST-GEN.6 — freezes DIST-GEN.5 §28/§53's peak-volume band
/// ([34,46] km/week) for the dark 15K × INTERMEDIATE × 4D second target.
/// Mirrors <see cref="TargetDistance16KPeakVolumeBandPolicy"/>'s own shape
/// exactly, as a distinct, separately-encoded C# constant (never a catalog
/// JSON row) — for the identical reason: the reused catalog identity is the
...
///
/// The numeric bounds are byte-identical to the 16K policy's own — a real,
/// disclosed evidence convergence (DIST-GEN.5 §28/§57), not template-copying
/// — kept as an independently-declared class rather than a shared constant
/// per DIST-GEN.6's own instruction against collapsing two independently-
/// authorized target cells into one literal.
/// </summary>
internal static class TargetDistance15KPeakVolumeBandPolicy
{
