...
/// <summary>
/// Backend Integration Phase 4D.4 regression guard: now that a CONCRETE
/// GoalFeasibilityResolver exists, re-prove it is not invoked anywhere in
/// live generation. Existing generate-preview/confirm behavior, including
/// Phase 0's no-silent-fallback guarantee, must remain byte-for-byte
/// unchanged.
/// </summary>
public sealed class GoalFeasibilityResolverNotWiredToGenerationTests
{
    ...
    [Fact]
    public async Task ExistingSupportedSeededTemplate_PreviewWithGoalEvidence_StillSucceeds_ResolverNeverInvoked()
    {
    ...
    [Fact]
    public async Task TenKIntermediate4Day_WithGoalEvidence_StillReturnsPlanTemplateNotAvailable()
    {
        await using var context = NewSeededContext();
        var service = NewServices(context);
    ...
    [Fact]
    public void PlanServices_ConstructorDependencies_DoNotIncludeGoalFeasibilityResolver()
    {
        ...
        Assert.DoesNotContain(constructorParamTypes, t => t == typeof(GoalFeasibilityResolver));
        Assert.DoesNotContain(constructorParamTypes, t => t == typeof(IGoalFeasibilityResolver));
    }

    [Fact]
    public void PlaceholderPlanGenerationEngine_ConstructorDependencies_DoNotIncludeGoalFeasibilityResolver()
    {
    ...
    [Fact]
    public void GeneratePreviewResponse_HasNoGoalFeasibilityResolutionProperty()
    {
