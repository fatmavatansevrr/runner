...
/// <summary>
/// Backend Integration Phase 4D.3 regression guard: now that a CONCRETE
/// CoreEntryReadinessResolver exists, re-prove it is not invoked anywhere in
...
/// unchanged.
/// </summary>
public sealed class CoreEntryReadinessResolverNotWiredToGenerationTests
{
    ...
    [Fact]
    public async Task TenKIntermediate4Day_WithReadinessEvidence_StillReturnsPlanTemplateNotAvailable()
    {
        await using var context = NewSeededContext();
        var service = NewServices(context);
    ...
    [Fact]
    public void PlanServices_ConstructorDependencies_DoNotIncludeCoreEntryReadinessResolver()
    {
        ...
        Assert.DoesNotContain(constructorParamTypes, t => t == typeof(CoreEntryReadinessResolver));
        Assert.DoesNotContain(constructorParamTypes, t => t == typeof(ICoreEntryReadinessResolver));
    }

    [Fact]
    public void PlaceholderPlanGenerationEngine_ConstructorDependencies_DoNotIncludeCoreEntryReadinessResolver()
    {
    ...
    [Fact]
    public void GeneratePreviewResponse_HasNoCoreEntryReadinessResolutionProperty()
    {
