...
/// <summary>
/// Backend Integration Phase 4D.1 regression guard: now that a CONCRETE
/// TimeAdequacyResolver exists (unlike Phase 4C, which only had interfaces),
/// re-prove it is not invoked anywhere in live generation. Existing
/// generate-preview/confirm behavior, including Phase 0's no-silent-fallback
/// guarantee, must remain byte-for-byte unchanged.
/// </summary>
public sealed class TimeAdequacyResolverNotWiredToGenerationTests
{
    ...
    [Fact]
    public async Task TenKIntermediate4Day_WithRaceDate_StillReturnsPlanTemplateNotAvailable()
    {
        await using var context = NewSeededContext();
        var service = NewServices(context);
    ...
    [Fact]
    public void PlanServices_ConstructorDependencies_DoNotIncludeTimeAdequacyResolver()
    {
        ...
        Assert.DoesNotContain(constructorParamTypes, t => t == typeof(TimeAdequacyResolver));
    }

    [Fact]
    public void PlaceholderPlanGenerationEngine_ConstructorDependencies_DoNotIncludeTimeAdequacyResolver()
    {
    ...
    [Fact]
    public void GeneratePreviewResponse_HasNoDecisionTraceProperty()
    {
        // Structural regression guard: RuntimeConditionResolutionResult / ResolverDecisionTrace
        // must never appear on a public response DTO in this phase.
