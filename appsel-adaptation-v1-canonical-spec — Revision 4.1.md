...
### Revizyon 4.1 — Rounding Tolerance + Target-Week Numeric Infeasibility (kapanış)
...
Sınıflandırma `Operational` kalsa da, `ScheduleRepair` seviyesinde
...
#### ROUNDING PRODUCT DEFAULT (YENİ — rev4.1, PRODUCT DEFAULT)
...
sweep sonucuna (maks. gözlemlenen %1.36) dayanan, kalibre edilecek
bir başlangıç değeridir.

Bu tolerans yalnızca test/governance kabul katmanında var olur.
Hiçbir runtime kodu bu değeri okumaz veya karşılaştırma yapmaz —
Maintain ve ProgressAsPlanned birbirini runtime'da hiç karşılaştırmaz,
her biri yukarıdaki formüle göre bağımsız hesaplanır.
```
...
**Sıradaki adım:** Adaptation V1'in numeric anchor/policy kapsamı bu
revizyonla kapanmıştır (§20, Phase 4M.4B.2C final report). Phase 4M.5
bu revizyonun onayından sonra ayrı bir kapsam kararıyla başlar.
